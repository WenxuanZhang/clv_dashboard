...
@views.route('/churn', methods=['GET', 'POST'])
def change_chart():
    feature = request.args['selected']
    #graphJSON = create_plot(feature,df_all)
    if feature == 'churn':
...
@views.route('/cba', methods=['GET', 'POST'])
def change_table():
...
#  behind change_features function, there is a ajax function, which capture the change from html
#  when user switch to another bar plot, it will be captured by ajax function
#  request.args return the selected feature, then it return the data which replace the 
#  bargraph chart plot 

# still got question: why does they use safe?


# how to selection based on data 
